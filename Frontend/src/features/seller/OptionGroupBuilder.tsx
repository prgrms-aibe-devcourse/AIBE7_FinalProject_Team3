  ...
  return (
    <>
      <div className="form-title-row">
        <div><h2>옵션 구성</h2><p>색상·사이즈에 한정하지 않고 필요한 기준을 직접 만듭니다.</p></div>
        <button type="button" className="secondary-button" onClick={() => setGroups((current) => [...current, { id: `group-${Date.now()}`, name: '', rawValues: '', values: [] }])}>＋ 그룹 추가</button>
      </div>
      ...
      <div className="sku-table">
        <div className="sku-heading"><strong>판매 조합(SKU)</strong><span>{combinations.length}개 조합</span></div>
